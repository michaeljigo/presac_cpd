% Purpose:  Initialize parameters for model optimization. This function contains an additional parameter specifying the criterion in a detection task (following SDT)
...
%           observer             structure of obsever parameters
%
% By:       Michael Jigo


function [stimdrive supdrive attn observer] = init_parameters_likelihood(varargin) 
...
in = {'cg_max'
      ...
      'observer'           % observer struct; start points, bounds
      'fix_with_input'};
...
bnds       = { [1 3]        [-1 0]           [1 3]        [-1 0]        [0.5 0.5]      [1 2]       [0 0]};
plaus_bnds = { [2 3]        [-0.3 -0.1]      [2 3]        [-0.7 -0.2]   [0.5 0.5]      [1 2]       [0 0]};
...
if isempty(start_params.stimdrive)
   ...
   for n = 1:numel(names)
      ...
      if isempty(start_params.(names{n}))
         ...
         stimdrive.(names{n}) = rand(1)*diff(plaus_bnds{n})+min(plaus_bnds{n});
      else
...
if isempty(start_params.supdrive)
   ...
   for n = 1:numel(names)
      ...
      if isempty(start_params.(names{n}))
         % randomly choose starting parameters
         supdrive.(names{n}) = rand(1)*diff(plaus_bnds{n})+min(plaus_bnds{n});
      else
...
bnds =       {[1 4]              [-1 -0.01]            [1 3]         [1 8]             [0 0]             [1 5]             [0.1 1]              [0.1 0.95]       [2 6]};
plaus_bnds = {[2 3]              [-0.2 -0.1]           [1 2]         [1 3]             [0 0]             [2 4]             [0.5 1]              [0.5 0.8]        [2 4]};
...
if isempty(start_params.attn)
   for n = 1:numel(names)
      ...
      if isempty(start_params.(names{n}))
         % randomly choose starting parameters
         attn.(names{n}) = rand(1)*diff(plaus_bnds{n})+min(plaus_bnds{n});
      else
         % use inputted start parameters
...
%% Observer parameters
% criterion    threshold for deciding to respond "yes" in a detection task

names =      {'criterion'};
bnds =       {[-1 1]};
plaus_bnds = {[-0.25 0.25]};


% put parameters into attn structure
if isempty(start_params.observer)
   for n = 1:numel(names)
      observer.bnd.(names{n}) = bnds{n};
      observer.plaus_bnd.(names{n}) = plaus_bnds{n};

      if isempty(start_params.(names{n}))
         % randomly choose starting parameters
         observer.(names{n}) = rand(1)*diff(plaus_bnds{n})+min(plaus_bnds{n});
      else
         % use inputted start parameters
         observer.(names{n}) = start_params.(names{n});
         
         if start_params.fix_with_input
            % this option will adjust the bounds so that the parameter value will remain fixed at what's inputted
            observer.bnd.(names{n}) = repmat(start_params.(names{n}),1,2);
            observer.plaus_bnd.(names{n}) = repmat(start_params.(names{n}),1,2);
         end
      end
   end
else
   observer = start_params.observer;
end
observer.param_names = names;
