...
function [cost dprime sse] = fit_task1_objective(stimdrive,supdrive,attn,observer,data,stim,config,params)
   ...
   %% evaluate moel on all densities and eccentricities 
      for d = 1:numel(data.density)
   ...
   %% scale dprime to match overall dprime (i.e., not density interactions)
      %modelOverall = mean(dprime(:));
      %scalar       = data.dprime.overall./modelOverall;
      %dprime       = dprime*scalar;

   %% scale separately for each density x ecc combination
      scalar         = mean(data.dprime.density_ecc.perf)./mean(dprime);
      dprime         = dprime.*scalar;

      % compute SSE
         sse       = nansum((dprime(:)-data.dprime.density_ecc.perf(:)).^2);
      ...
      for d = 1:numel(density)
         for e = 1:numel(ecc)
            % extract hits and false alarms for this condition
               hits     = sum(data.trials.presence==1 & data.trials.response==1 & data.trials.density==density(d) & data.trials.ecc==ecc(e)); % # hits
               fa       = sum(data.trials.presence==0 & data.trials.response==1 & data.trials.density==density(d) & data.trials.ecc==ecc(e)); % # false alarms
               pres     = sum(data.trials.presence==1 & data.trials.density==density(d) & data.trials.ecc==ecc(e));                           % # target present trials
               abse     = sum(data.trials.presence==0 & data.trials.density==density(d) & data.trials.ecc==ecc(e));                           % # target absent trials
   ...
   %% save interim fit parameters
      ...
      out.cost          = cost;
      out.sse           = sse;
      savedir = sprintf('../data/running_parameters/v3/');
