% Purpose:  Objective function for fitting behavioral performance from Task 3.
% By:       Michael Jigo
%           05.06.21

function [cost dprime] = model_task3_objective(stimdrive,supdrive,subjdata,energy,params,freeparams,compute_cost)
   ...
   for d = 1:numel(subjdata.densityvals)
      ...
      scalar(d,:) = repmat(mean(subjdata.dprime.targecc.perf(d,:))./mean(dprime(d,:)),1,numel(subjdata.eccvals));
   end

   % scale dprime
   dprime = dprime./mean(dprime(:)).*mean(subjdata.dprime.targecc.perf(:));
   %dprime = dprime.*scalar;

   % compute SSE
   if compute_cost
      obsdata = subjdata.dprime.targecc.perf(:);
