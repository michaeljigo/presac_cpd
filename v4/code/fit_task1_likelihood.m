...
function fit_task1_likelihood(subj)
   ...
   %% Load data
      ...
      % keep only size 5 and 11
         keepIdx  = tmp(:,idx_size)==5 | tmp(:,idx_size)==11;
         tmp      = tmp(keepIdx,:);
      ...
      % add in calculated dprime
         ...
         % keep dprime from only size 5 and 11
            allSizes = cell2mat(cellfun(@str2num,data.dprime.size_ecc.factorLabels.factor2,'un',0));
            keepIdx  = allSizes==5 | allSizes==11;
            data.dprime.size_ecc.perf = data.dprime.size_ecc.perf(keepIdx,:);
   ...
   %% Evaluate model to generate best-fitting values at finely-spaced eccentricities
      ...
      % evaluate at finely spaced eccentricities
         data.fineEcc      = min(data.ecc):0.25:max(data.ecc);
