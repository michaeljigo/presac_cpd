...
function display_model_fits(subj)
   ...
   %% Load fitted parameters and associated data
      dataDir = sprintf('../data/fitted_parameters/task4/%s.mat',subj);
      load(dataDir);

   %% Display each size in the same plot
      colors = [5 113 176; 202 0 32]./255;
      figure('name','Size fits','position',[1 1 899 822]);
      for s = 1:numel(out.data.size)
         subplot(1,numel(out.data.size),s)
         % display fit
         ...
         % pretty up figure
            ...
            title(sprintf('size=%i',out.data.size(s)),'fontname','arial','fontsize',10);
      end
